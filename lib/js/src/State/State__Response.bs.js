...
function canonicalizeEscape(content) {
  return Js_string.replaceByRe(/\\n|\\r\\n/g, "\n", content);
}

function insertSpaces(offset, content) {
  return Js_string.replaceByRe(/\n/g, "\n" + Js_string.repeat(offset, " "), content);
}
...
function handle$1(state, dispatchCommand, response) {
    ...
    switch (response.TAG | 0) {
      ...
      case /* GiveAction */6 :
          ...
          if (goal !== undefined) {
            if (typeof give === "number") {
              ...
              var content = give._0;
              var goalStart = goal.interval[0];
              var match = Tokens$AgdaModeVscode.lookupLocalSrcLoc(state.tokens, goalStart);
              var columnOffset = match !== undefined ? match[1].start.character : 0;
              handleResponse = $$Promise.map($$Promise.flatMap(State__Goal$AgdaModeVscode.modify(state, goal, (function (param) {
                              return insertSpaces(1 + columnOffset | 0, canonicalizeEscape(content));
                            })), (function (param) {
                          return State__Goal$AgdaModeVscode.removeBoundaryAndDestroy(state, goal);
                        })), (function (param) {
                      return {
      ...
      case /* RunningInfo */10 :
          ...
          if (verbosity !== 1) {
            ...
            var message$1 = removeNewlines(canonicalizeEscape(response._1));
  ...
  return $$Promise.tapOk(handleResponse, (function (param) {
                Chan$AgdaModeVscode.emit(state.channels.responseHandled, response);
              }));
}

exports.canonicalizeEscape = canonicalizeEscape;
exports.insertSpaces = insertSpaces;
